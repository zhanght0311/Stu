<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="T1_menu.aspx.cs" Inherits="Stud.T1_menu" %>
 ...
 <!--头部-->
    ...
    <div position="left"  title="快捷菜单" id="global_left_nav"> 
            <div title="信息录入" iconcss="menu-icon-model" class="l-scroll">
                ...
                <% if (true) 
                { %>
                    <li><a class="l-link" href="javascript:f_addTab('listpage1','科研情况录入','teacher1/T1_1.aspx')">科研情况录入</a></li>
               <%}%>
                    <% if (true) 
                { %>
                    <li><a class="l-link" href="javascript:f_addTab('listpage8','管理科研录入','teacher1/T1_6.aspx')">管理科研录入</a></li>
               <%}%>
               <% if (true) 
                { %>
                    <li><a class="l-link" href="javascript:f_addTab('listpage2','比赛情况录入','teacher1/T1_2.aspx')">比赛情况录入</a></li>
                <%}%>
                     <% if (true) 
                { %>
                    <li><a class="l-link" href="javascript:f_addTab('listpage9','管理比赛录入','teacher1/T1_7.aspx')">管理比赛录入</a></li>
               <%}%>
                </ul>
            </div>
        <div title="学生自评查看" iconcss="menu-icon-model" class="l-scroll">
                <ul class="nlist">
                    <% if (true) 
                { %>
                    <li><a class="l-link" href="javascript:f_addTab('listpage5','未自评学生','teacher1/T1_5.aspx')">未自评学生</a></li>
                <%}%>
                   <% if (true) 
                { %>
                    <li><a class="l-link" href="javascript:f_addTab('listpage7','已自评学生查看','teacher1/T1_8.aspx')">已自评学生查看</a></li>
                <%}%>
                </ul>
            </div>
            <div title="学生评价" iconcss="menu-icon-model" class="l-scroll">
                <ul class="nlist">
                    <% if (true) 
                { %>
                    <li><a class="l-link" href="javascript:f_addTab('listpage3','以评价学生','teacher1/T1_3.aspx')">已评价学生</a></li>
                <%}%>
                <%if (true)
                  { %>
                   <li><a class="l-link" href="javascript:f_addTab('listpage4','待评价学生','teacher1/T1_4.aspx')">待评价学生</a></li>
                   <%} %>
